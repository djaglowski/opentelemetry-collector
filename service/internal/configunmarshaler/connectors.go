...
type Connectors struct {
	conns map[component.ID]component.ConnectorConfig
...
func (c *Connectors) Unmarshal(conf *confmap.Conf) error {
	...
	c.conns = make(map[component.ID]component.ConnectorConfig)
	...
	for id, value := range rawConns {
		...
		if err := component.UnmarshalConnectorConfig(confmap.NewFromStringMap(value), connectorCfg); err != nil {
...
func (c *Connectors) GetConnectors() map[component.ID]component.ConnectorConfig {
	return c.conns
}
