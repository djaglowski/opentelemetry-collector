...
func TestCount(t *testing.T) {
	...
	countsSink := new(connectortest.MetricsConsumerSink)

	// spansCounter, err := f.CreateTracesToMetrics(ctx, set, cfg, countsSink)
	// assert.NoError(t, err)
	// assert.NotNil(t, spansCounter)

	mcm := connector.NewMetricsConsumerMap(countsSink)

	metricsCounter, err := f.CreateMetricsToMetrics(ctx, set, cfg, mcm)
	assert.NoError(t, err)
	assert.NotNil(t, metricsCounter)

	logsCounter, err := f.CreateLogsToMetrics(ctx, set, cfg, mcm)
	assert.NoError(t, err)
	...
	assert.Equal(t, 2, len(countsSink.AllMetrics()))
	// assertEqualCounts(t, spanCounts, "span.count", countsSink.AllMetrics()[0].ResourceMetrics())
	assertEqualCounts(t, metricCounts, "metric.count", countsSink.AllMetrics()[0].ResourceMetrics())
	assertEqualCounts(t, logCounts, "log.count", countsSink.AllMetrics()[1].ResourceMetrics())
